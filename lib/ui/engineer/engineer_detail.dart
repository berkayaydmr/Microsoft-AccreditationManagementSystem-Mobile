...
class _EngineerViewState extends State<EngineerDetailView>{
        ...
        children: [
          Column(
            children: [
              BlocBuilder<EngineerBloc, EngineerState>(
                bloc: _engineersDetailBloc,builder: (context,state){
                if (state is EngineerDetailSuccess) {
                  username = state.engineer.name!;
                  return HeadTitleWidget(titles: ["Engineers",username], hideButtons: true,);
                }
                          ...
                          return Column(
                            ...
                            children: [
                              ...
                              if(state.engineer.accreditations!.isNotEmpty)
                                SizedBox(
                                    ...
                                    child: ListView.builder(
                                        ...
                                        itemCount: state.engineer.accreditations!.length,
                                        itemBuilder: (context,index){
                                          return Padding(
                                            padding: context.edgeLow,
                                            child: FilterChip(label: Text("MIP"), onSelected: (bool value) {  },),
                                          );
                                        }),
                                  ),
                              if(state.engineer.accreditations!.isEmpty)
                                SizedBox(
                                    height: context.height * 0.1,
                                    child:Center(child: Text("No Accreditations"),)),
                            ],
                          );
                        }
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                IconButton.outlined(onPressed: _alert, icon: Icon(Icons.delete), color: MicrosoftColor.RED,),
                ElevatedButton(onPressed: _switchEngineer,
                  child: BlocBuilder<EngineerBloc, EngineerState>(
                    bloc: _engineersDetailBloc,
                    builder: (context, state) {
                      if(state is EngineerDetailSuccess){
                        username = state.engineer.name!;
                        return Text("Continue as $username", style: context.textTheme.bodyMedium!.copyWith(color: Colors.white, fontWeight: FontWeight.bold),);
                      }
                      return const LoadingIndicator();
                    },
                  ),),
                IconButton.outlined(onPressed: _editEngineer, icon: Icon(Icons.edit), color: MicrosoftColor.BLUE,),
              ],
            ),
          ),
        ],
  ...
  void _switchEngineer(){
    LocalStorageManager.setString(LocalStorageConstants.USERID, widget.engineerID.toString());
    context.router.replaceAll([DashboardViewRoute()]);
  }
  ...
  void _alert(){
    AlertDialog alertDialog = AlertDialog(title: Text("Delete Engineer"), content: Text("Are you sure you want to delete this engineer?"), actions: [
